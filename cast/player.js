...
sampleplayer.CastPlayer = function(element) {
 ...
 /**
  * The preloader for images.
  */
  this.imagePreloader_ = this.receiverManager_.getCastMessageBus('urn:x-cast:no.jottacloud.jottacloudphotos.images');
  this.imagePreloader_.onMessage = this.onMessage_.bind(this);
...
function calculateAspectRatioFit(srcWidth, srcHeight) {
    var ratio = Math.min(screen.width / srcWidth, screen.height / srcHeight);
...
function modify_image(image_src){
  ...
  sampleplayer.fadeOut_(androidImage, 0.3);
   i.onload = function() {
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
  document.getElementById("androidImage").style.display='none';
...
sampleplayer.CastPlayer.prototype.onMessage_ = function(event) {
  var incomingJson = JSON.parse(event.data);
  for(var i = 0; i < incomingJson.images.length; i++) {
    var imageURL = incomingJson.images[i];
    var loadImage = new Image();
    loadImage.src = imageURL.url;
    this.log_('loaded ' + imageURL.url);
  }
};
...
sampleplayer.CastPlayer.prototype.onGetStatus_ = function(event) {
  this.log_('onGetStatus');
  this.onGetStatusOrig_(event);
...
sampleplayer.getType_ = function(media) {
  ...
  if (contentType.indexOf('audio/') === 0) {
    ...
    return sampleplayer.Type.IMAGE;
    ...
    return sampleplayer.Type.IMAGE;
  } else if(contentType.indexOf('image/heif') === 0) {
    return sampleplayer.Type.IMAGE;
  } else if(contentType.indexOf('image/heic') === 0) {
    return sampleplayer.Type.IMAGE;
...
sampleplayer.fadeTo_ = function(element, opacity, time, opt_doneFunc) {
  ...
  // webkitTransition callback was the source of the earlier crashes (also in
